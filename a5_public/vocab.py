...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        int_sentence = self.words2charindices(sents)
        pad_int_sentence = pad_sents_char(int_sentence, char_pad_token = self.char2id['<pad>'])
        sents_var = torch.tensor(pad_int_sentence, device = device, dtype=torch.long).permute(1,0,2)
        return sents_var
