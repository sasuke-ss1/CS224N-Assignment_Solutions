...
class CharDecoder(nn.Module):
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        loss_fn = nn.CrossEntropyLoss(reduction = "sum",ignore_index=self.target_vocab.char2id["<pad>"])
        loss = loss_fn(s_t.permute(1,2,0),char_sequence[1:].permute(1,0))
        return loss
        ### END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        output_word = []
        curr_hidden = initialStates
        batch_size = initialStates[0].shape[1]
       
        curr_char = torch.tensor([[self.target_vocab.start_of_word]*batch_size], device = device)
        for i in range(max_length):
           s_t ,curr_hidden = self.forward(curr_char, curr_hidden)
           p_t = torch.softmax(s_t, dim = 2)
           _, curr_char = torch.max(p_t, dim = 2)
           output_word += [curr_char]
        decodedWords = []
        output_word = torch.cat(output_word).t().tolist()
        for words in output_word:
            word = ""
            for char in words:
